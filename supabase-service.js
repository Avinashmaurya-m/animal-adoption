...
export async function createPost(data) {
  ...
  const payload = {
    owner_id: user.id,
    author: data.author || data.authorName || profile?.name || user.email,
    author_name: data.authorName || data.author || profile?.name || user.email,
    kind: data.kind || "adoption",
    pet: animalValue,
    animal_type: animalValue,
    name: (data.name || animalValue).trim(),
    breed: (data.breed || "").trim(),
    age: (data.age || "").trim(),
    gender: (data.gender || "").trim(),
    city: (data.city || locationValue).trim(),
...
export async function publishPost({ kind, pet, location, text, imageFile, animalType, name, breed, age, gender, city, health, vaccinated, phone, description, authorName }) {
  ...
  const payload = {
    owner_id: user.id,
    author: authorName || profile?.name || user.email,
    author_name: authorName || profile?.name || user.email,
    kind: kind || "adoption",
    pet: pet || animalType || "Dog",
    animal_type: animalType || pet || "Dog",
    name: name || pet || animalType || "Animal",
    breed: breed || "",
    age: age || "",
    gender: gender || "",
    city: city || location || "",
